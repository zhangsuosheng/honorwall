...
function show(page) {
    ...
    for(var j=begin;j<=end;j++) {
        ...
        var mess="";
        if(count==1) {
                mess = '<tr class="success"><th>' + j + '</th><th>' + all[j-1]["id"] + '</th><th>' + all[j-1]["name"] + '</th><th>' + all[j-1]["grade"] + '</th><th>' + all[j-1]["field"] +'</th><th>' + all[j-1]["classroom"] + '</th><th><a href="./studentmessagechange_2/id='+ all[j-1]["id"]+ '"'+' class="btn btn-xs btn-warning" >账户信息修改</a></th>';
        }
        else {
            mess = '<tr><th>' + j + '</th><th>' + all[j-1]["id"] + '</th><th>' + all[j-1]["name"] + '</th><th>' + all[j-1]["grade"] + '</th><th>' + all[j-1]["field"] +'</th><th>' + all[j-1]["classroom"] + '</th><th><a href="./studentmessagechange_2/id='+ all[j-1]["id"]+ '"'+ 'class="btn btn-xs btn-warning" >账户信息修改</a></th>';
        }
        html_middle += mess;
    }
    if (page<=7){
